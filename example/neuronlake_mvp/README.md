...
The server uses `NEURONLAKE_DEFAULT_MAX_TOKENS` as both the fallback and upper bound for client `max_tokens`; `mise.toml` sets this to `192` so llama.cpp does not run unbounded local generations.
